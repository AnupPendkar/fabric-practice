...
import { ControllerBaseClass } from "./control-base.class";

export class ControllerRendererClass extends ControllerBaseClass {
  // View Canvas Ref
  viewCanvasRef: any;
  constrollerSettings: ControllerSettings;
  objectsGeneratorInstanceManageClass =
    new ObjectsGeneratorInstanceManageClass();

  isZoomEnabled: boolean;

  backGroundImageRef: FabricImage;
  heapMapBackgroundRef: FabricImage;

  onAfterCanvasEmitterFunc: () => void;

  delta = new Point(0, 0);
  constructor(func: () => void) {
    super();
    this.onAfterCanvasEmitterFunc = func;
  }
  // fabric constructor
  ...
  get getCustomCanvasConfig(): CustomCanvasConfig {
    const config: CustomCanvasConfig = {
      canvas_id: "12",
      canvasHeight: this.canvasWrapperRef?.offsetHeight,
      canvasWidth: this.canvasWrapperRef?.offsetWidth,
  ...
  setToggleBackground(ctrBGView: CtrBGView, path: string): void {
    FabricImage.fromURL(path).then((image) => {
      let scaleX = this.canvasRef?.width / image.width;
      let scaleY = this.canvasRef?.height / image.height;
      let X = 0;
      let Y = 0;
  ...
  onSetCanvasConfigAndRender(background?: CtrBGView): void {
    this.canvasRef = new Canvas(this.viewCanvasRef, {
  ...
  registerCanvasEvents(): void {
    ...
    this.canvasRef.on("mouse:up", (e: any) => {
  ...
  onCanvasMouseDown(e: any): void {
    ...
    if (
      this.constrollerSettings.controllerView ===
      ControllerViewType.DRAWING_MODE
    ) {
  ...
  onCanvasMouseMove(e: any): void {
    const pointer = this.canvasRef.getPointer(e.e);
    const activeObject = this.canvasRef.getActiveObject() as Rect;
    if (activeObject) {
  ...
  onTransformCanvas(e: WheelEvent): void {
    ...
    zoom *= 0.999 ** delta;
    this.canvasRef.zoomToPoint({ x: e.offsetX, y: e.offsetY } as any, zoom);
    this.initialZoom = zoom;
    e?.preventDefault();
    e?.stopPropagation();
    this.canvasRef.renderAll();
  }
  onPanCanvas(e: any): void {
    this.currentX = e.e.x;
    this.currentY = e.e.y;
    ...
    if (canvas) {
      if (
        Math.abs(this.currentX - this.lastX) <= 100 &&
        Math.abs(this.currentY - this.lastY) <= 100
      ) {
  ...
  onRegisterMobileGuesturesEvents(): void {
    ...
    this.canvasRef.on("touch:gesture", (e) => {
      ...
      if (evt?.e?.touches && evt?.e?.touches.length === 2) {
        this.pausePanning = true;
        const point = new Point(evt.self?.x, evt.self?.y);
    ...
    this.canvasRef.on("touch:drag", (e) => {
      ...
      if (
        ...
        if (
          ...
          const delta = new Point(this.xChange, this.yChange);
  ...
  private _avoidObjectMovingOutsideOfBoundaries(e: any): void {
    const obj = e.target;
